...
function ScreenRecorder({
  ...
  const startScreenRecording = async () => {
    try {
      ...
      toast.error(isChinese ? "无法开始屏幕录制" : "Could not start screen recording");
      throw error;
    }
  };
  ...
  const kickoffGeneration = () => {
    if (screenRecordingDataUrl) {
      ...
      toast.error(isChinese ? "不存在屏幕录制, 请再试一次" : "Screen recording does not exist. Please try again.");
      throw new Error(isChinese ? "没有录屏数据" : "No screen recording data url");
    }
  };
  ...
  return (
    <div className="flex items-center justify-center my-3">
      {screenRecorderState === ScreenRecorderState.INITIAL && (
        <Button onClick={startScreenRecording}>{isChinese ? "录屏" : "Record Screen"}</Button>
      )}
      ...
      {screenRecorderState === ScreenRecorderState.RECORDING && (
        <div className="flex items-center flex-col gap-y-4">
          <div className="flex items-center mr-2 text-xl gap-x-1">
            ...
            <span>{isChinese ? "录制中..." : "Recording..."}</span>
          </div>
      ...
      {screenRecorderState === ScreenRecorderState.FINISHED && (
        <div className="flex items-center flex-col gap-y-4">
          <div className="flex items-center mr-2 text-xl gap-x-1">
            <span>{isChinese ? "屏幕录制已捕获" : "Screen Recording Captured."}</span>
          </div>
          ...
          <div className="flex gap-x-2">
            ...
            >
              {isChinese ? "重新录制" : "Re-record"}
            </Button>
            <Button onClick={kickoffGeneration}>{isChinese? "生成" : "Generate"}</Button>
          </div>
        </div>
      )}
    </div>
